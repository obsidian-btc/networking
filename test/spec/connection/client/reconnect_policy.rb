...
context 'Client Connection' do
  context 'Never reconnect policy' do
    policy = Connection::Connection::Client::ReconnectPolicy::Never.new
  ...
  context 'When Closed reconnect policy' do
    policy = Connection::Connection::Client::ReconnectPolicy::WhenClosed.new
  ...
  context 'Resolving a reconnect policy' do
    test 'Exists' do
      policy = Connection::Connection::Client::ReconnectPolicy.get :never

      assert policy.is_a?(Connection::Connection::Client::ReconnectPolicy)
    end

    test 'Does not exist' do
      begin
        Connection::Connection::Client::ReconnectPolicy.get :not_a_policy
      rescue Connection::Connection::Client::ReconnectPolicy::Error => error
      end
